...
# For manual database setup, see [Manual PostgreSQL Setup](./deployment/manual-deployment.md#manual-postgresql-setup).

Follow the instructions in [PostgreSQL Database Setup](./postgres-setup.md).
...
- [Step 5.1: Create Namespace](./deployment/manual-deployment.md#step-51-create-namespace)
- [Step 5.2: Create Secrets and ConfigMaps](./deployment/manual-deployment.md#step-52-create-secrets-and-configmaps)
- [Step 5.3: Deploy the LiteLLM Helm Chart](./deployment/manual-deployment.md#step-53-deploy-the-litellm-helm-chart)

### 3. Verify the Standalone LiteLLM Instance

Before proceeding, use the verification commands from [Step 5.5](./deployment/manual-deployment.md#step-55-verify-deployment-and-access-ui) to ensure LiteLLM is running correctly and you can access its UI. The dashboard will show no traffic, which is expected.
...
Create the integration secret by following the instructions in [Step 5.4.1: Create the Integration Secret](./deployment/manual-deployment.md#541-create-the-integration-secret).

#### 4.2. Update CodeMie API Configuration

In your `codemie-api/values-<cloud>.yaml`, remove the old DIAL environment variables and ensure the LiteLLM variables (as specified in [Configure Values](./configure-values.md)) are correctly configured.
...
Redeploy the CodeMie API to apply the changes by following the instructions in [Step 5.4.2: Redeploy the CodeMie API](./deployment/manual-deployment.md#542-redeploy-the-codemie-api).
...
- Return to [LiteLLM Proxy Overview](./index.md)
