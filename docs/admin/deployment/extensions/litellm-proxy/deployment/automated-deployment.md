...
2. **Configure API and Proxy Values:** Update both the `codemie-api/values-<cloud>.yaml` and `litellm/values-<cloud_name>.yaml` files with your environment-specific settings as described in [Configure Values](../configure-values.md).

3. **Configure LLM Models:** Review and customize your desired models and regions in the `litellm/config/litellm-<cloud>-config.yaml` file, as detailed in [Model Configuration](../model-config.md).
