...
1. **[PostgreSQL Database Setup](./postgres-setup.md)** — Create a dedicated database and user for LiteLLM.
2. **[Configure API and Proxy Values](./configure-values.md)** — Configure the required `values.yaml` files for CodeMie and LiteLLM.
3. **[Cloud Provider Authentication](./auth-secrets.md)** — Configure credentials for AWS, Azure, and GCP.
4. **[LiteLLM Model Configuration](./model-config.md)** — Configure model lists and regions.
5. **[Deployment](./deployment/index.md)** — Install the LiteLLM Proxy using one of two methods:
   - Automated Deployment (Recommended)
   - Manual Deployment

:::info PostgreSQL Database Setup

The `postgres_litellm` database and user are created automatically during deployment via the `db-init-job`. For manual setup instructions, see [Manual PostgreSQL Setup](./deployment/manual-deployment.md#manual-postgresql-setup).

:::

For users migrating from the deprecated DIAL Proxy, see:

- **[Migration Guide from DIAL to LiteLLM](./migration-guide.md)**

## Next Steps

Follow the deployment workflow starting with [Configure API and Proxy Values](./configure-values.md).
